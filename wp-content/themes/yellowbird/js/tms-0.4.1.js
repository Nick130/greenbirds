...
jQuery(document).ready(function(){
	jQuery('.tmslider')._TMS({
		show: (tm_setting.show == '') ? 0 : tm_setting.show,
		pauseOnHover: (tm_setting.pauseonhover == 'true') ? true : false,
		prevBu: '.prev',
		nextBu: '.next',
		duration: (tm_setting.duration == '') ? 10000 : tm_setting.duration,
		preset: (tm_setting.preset == '') ? 'zoomer' : tm_setting.preset,
		pagination: false,	//'.pagination',true,'<ul></ul>'
		slideshow: (tm_setting.slideshow == '') ? 7000 : tm_setting.slideshow,
		progressBar: false
	});
});